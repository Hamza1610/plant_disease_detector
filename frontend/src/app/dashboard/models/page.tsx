          ...
          {models.map((model) => (
            <div key={model.model_id} className="glass-panel rounded-2xl p-6 flex flex-col hover:-translate-y-1 transition-transform duration-300">
              ...
              <div className="flex flex-col gap-3">
                  <Link href={`/dashboard/predict?model=${model.model_id}`} className="w-full text-center py-2.5 bg-green-500 hover:bg-green-400 text-black font-semibold rounded-xl transition-colors">
                     Deploy in Studio
                  </Link>
                  <Link href={`/dashboard/benchmarks`} className="w-full text-center py-2.5 bg-white/5 hover:bg-white/10 text-white font-medium rounded-xl border border-white/10 transition-colors">
                    View Benchmarks
                 </Link>
              </div>
            </div>
