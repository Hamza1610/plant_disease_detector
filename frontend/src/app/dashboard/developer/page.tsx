...
export default function DeveloperDashboard() {
  ...
  const [metrics, setMetrics] = useState<{ total_scans: number; average_confidence: number } | null>(null);

  const fetchMetrics = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/analytics/developer/summary`, {
    ...
    <div className="min-h-screen bg-[#050505] text-white p-4 md:p-8">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {[
              { label: 'Total Inferences', value: metrics ? metrics.total_scans.toLocaleString() : '...', icon: BarChart3, color: 'text-blue-400' },
              { label: 'Active Models', value: models.length, icon: Database, color: 'text-green-400' },
              { label: 'Average Accuracy', value: metrics ? `${metrics.average_confidence}%` : '...', icon: Globe, color: 'text-purple-400' },
            ].map((stat, i) => (
          ...
          {/* Models List */}
          <div className="bg-white/[0.03] border border-white/10 rounded-3xl overflow-hidden h-fit">
